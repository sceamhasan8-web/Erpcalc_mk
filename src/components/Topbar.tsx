...
import { Sun, Moon, Search, Bell, Menu, LogOut, ChevronDown, Check, Shield } from 'lucide-react';
    ...
    <header className="app-header flex w-full items-center justify-between gap-3 relative z-20">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="h-6 w-6 rounded-lg bg-gradient-to-tr from-cyan-400 to-blue-300 text-slate-900 font-bold text-xs flex items-center justify-center shadow-sm">
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-[var(--ec-border)] bg-[var(--ec-card)] shadow-2xl p-2 z-50 text-[var(--ec-foreground)] animate-in fade-in zoom-in-95 duration-100">
              <div className="px-3 py-2 border-b border-[var(--ec-border)] mb-1">
                <div className="font-semibold text-xs text-[var(--ec-foreground)]">{user?.name}</div>
                <div className="text-[11px] text-[var(--ec-muted)] truncate">{user?.email}</div>
                <div className="mt-1.5 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
